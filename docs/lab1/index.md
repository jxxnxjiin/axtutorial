...
이 실습에서 쓰는 업무 유형 — `자료 요약`(취합·통합) `확인하고 수정하기`(검증·검수) · [여섯 가지 유형 다시 보기](../types/index.md)
...
이 첫 실습에서는 그 일을 AI에게 통째로 맡기는 법을 배웁니다. 빠뜨리지 않는다, 맘대로 바꾸지 않는다, 없는 건 지어내지 않는다. 이 세 원칙을 규칙으로 넘기면 됩니다. 모든 주문 건을 종합한 엑셀 파일이 완성되면, 새거나 겹친 건은 없는지 **검산**까지 해볼 것입니다.
...
| 단계 | 할 일 |
|---|---|
| [1단계](step1.md) | 세 파일의 내용을 **하나의 표**로 정리한다 |
