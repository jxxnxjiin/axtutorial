...
이 실습에서 쓰는 업무 유형 — `모으기`(취합·통합) `줄이기`(집계·분석) `검사하기`(검증·검수) · [여섯 가지 유형 다시 보기](../types/index.md)
...
쿠폰·이벤트를 뿌리기는 쉬워도 어떤 게 남는 장사였는지는 아무도 모르는 상황입니다. 마케팅 부서에서 흔히 생기는 일입니다. [실습 1](../lab1/index.md)·[실습 2](../lab2/index.md)에서 배운 데이터 모으기로 다섯 파일을 하나로 만들고, 이번에 새로 배우는 자료 요약으로 9천여 건을 쿠폰 44줄로 줄입니다.

그런데 표를 들고 가면 팀장님은 "그래서 이 쿠폰들, 왜 적자야?"라고 묻습니다. 숫자는 '무엇'만 보여주고 '왜'는 안 알려주니, 마지막에는 표를 해석해 "그래서 뭘 해야 하는지" **인사이트**까지 뽑습니다. 끝나면 검산까지 마친 성과표와 행동 제안이 남습니다.
