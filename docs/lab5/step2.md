...
1단계에서 한 건이 기준대로 분류됐습니다. 기준이 통한다는 걸 확인했으니 이제 520건 전체를 같은 규칙으로 분류해 빈 표를 채웁니다. 사람이 한 건에 10초씩만 써도 520건이면 대략 한 시간 반이지만 AI는 몇 분이면 끝냅니다. 양이 늘어날수록 이 차이는 더 벌어질 것입니다. 단, AI는 규칙을 준 만큼만 정확합니다. 아래 체크리스트가 그 규칙의 재료입니다.
