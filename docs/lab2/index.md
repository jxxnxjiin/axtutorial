...
[실습 1](../lab1/index.md)에서는 이미 표였던 것들을 합쳤지만, 이번에는 표가 아예 없는 줄글 계약서에서 데이터를 뽑아 만듭니다. 계약서·공문·회의록처럼 문서 더미에서 항목을 추려 표로 만드는 일은 어느 부서에나 있고, 사람이 하면 파일을 하나씩 열어 "계약 끝나는 날이 언제지, 금액이 얼마지" 찾아야 합니다.

주의해야 할 지점은 문서에 없는 값을 AI가 그럴듯하게 지어낼 수 있다는 것입니다. PDF에서 뽑을 항목과 그 규칙을 정해 AI에게 맡기고, 결과가 믿을 만한지 **검증**까지 마치면 흩어져 있던 계약서의 내용을 하나로 종합한 엑셀 파일이 완성됩니다.
...
| 단계 | 할 일 |
|---|---|
| [1단계](step1.md) | 계약서 8건에서 6개 항목을 뽑아 **하나의 표**로 정리한다 |
| [2단계](step2.md) | 종합된 결과를 원본과 대조하며 **검산**한다 |
